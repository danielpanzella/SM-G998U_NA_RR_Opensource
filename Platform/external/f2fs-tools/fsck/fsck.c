			...
			struct node_info *ni)
{
	...
	if ((c.feature & cpu_to_le32(F2FS_FEATURE_COMPRESSION)) && ntype == TYPE_INODE &&
		...
		if ((node_blk->i.i_compress_algrithm >= COMPRESS_MAX) ||
		...
		u32 *blk_cnt, struct node_info *ni, struct child_info *child_d)
{
	...
	if (node_blk->i.i_flags & cpu_to_le32(F2FS_CASEFOLD_FL) &&
			(ftype != F2FS_FT_DIR || !(c.feature & cpu_to_le32(F2FS_FEATURE_CASEFOLD)))) {
		if (c.fix_on) {
			node_blk->i.i_flags &= cpu_to_le32(~F2FS_CASEFOLD_FL);
			FIX_MSG("[0x%x] inode has wrong casefold flag. remove it!", nid);
			need_fix = 1;
		}
	}
...
void print_fsck_return_value()
{
	if (!c.exit_code)
		stlog("return: clean\n");
	else if (c.exit_code == F2FS_FSCK_FULL_CHECKED)
		stlog("return: force and clean\n");
	else if (c.exit_code == (F2FS_FSCK_FULL_CHECKED | F2FS_FSCK_MODIFIED))
		stlog("return: force and modified\n");
	else
		stlog("return: invalid value\n");
}
