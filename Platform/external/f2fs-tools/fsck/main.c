...
void f2fs_parse_options(int argc, char *argv[])
{
	...
	if (!strcmp("fsck.f2fs", prog)) {
		const char *option_string = ":aC:c:m:d:fg:l:O:p:q:StyV";
		int opt = 0, val;
		char *token;
		...
		const char *option_string = "d:sf:t:iV";
		...
		const char *option_string = "C:d:f:p:s:St:T:X:r:V";
...
fsck_again:
	...
	if (ret != 0) {
		if (ret == 1) {
			...
			c.elapsed_time = (get_boottime_ns() - start) / 1000000000.0;
